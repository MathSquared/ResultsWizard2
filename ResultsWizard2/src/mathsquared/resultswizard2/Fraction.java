...
public class Fraction implements Comparable<Fraction> { // TODO write unit tests
     ...
     * 
     * <code>
     * number = { "0" } , ? <i>a Java decimal integer literal that represents a valid positive int</i> ?<br />
     * denominator = { "0" } , ? <i>a Java decimal integer literal that represents a valid positive int other than 0</i> ?<br />
     * unit-symbol = { " " } , ( "u" | "U" | "+" | " " ) , { " " }<br />
     * fraction-symbol = { " " } , "/" , { " " }<br />
     * decimal = { "0" } , ? <i>a Java decimal integer literal that represents a valid positive int, with the exception that it contains exactly one decimal point inserted between two arbitrary characters or at the start or end of the string</i> ? , { "0" }<br />
